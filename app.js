...
function renderAgenda() {
  ...
  return `<div class="agenda-grid">
    ...
    <div class="card"><div class="card-title">➕ Journées supplémentaires</div>
      ${(DB.extraVisits || []).length ? DB.extraVisits.sort((a, b) => new Date(a.date || 0) - new Date(b.date || 0)).map(v => `<div class="doc-item" onclick="showExtraVisitModal('${v._id}')" style="cursor:pointer;"><div class="doc-icon">${v.cancelled ? '❌' : '👩‍👦'}</div><div class="doc-info"><div class="name">${fmtLong(v.date)}${v.cancelled ? ' <span style="color:var(--danger);font-size:11px;">(annulé' + (v.note ? ': ' + v.note : '') + ')</span>' : ''}</div><div class="meta">${v.cancelled ? '' : (v.time || '')}${!v.cancelled && v.note ? ' · ' + v.note : ''}</div></div><button class="btn-del" onclick="event.stopPropagation();delFrom(DB.extraVisits,'${v._id}')">×</button></div>`).join('') : '<div class="empty" style="padding:8px"><div class="sub">Ajoute des jours hors dimanche (mercredi, vacances...)</div></div>'}
      <button class="btn btn-outline btn-full btn-sm" style="margin-top:8px" data-action="add-extra-visit-new">+ Journée</button></div>
    ${renderPastSundays()}
  </div>`;
}
