...
function updateFab() {
  ...
  if (S.screen === 'agenda') { add('📝', 'Note dimanche', 'add-sunday-note'); add('➕', 'Journée supp.', 'add-extra-visit'); add('🩺', 'RDV', 'add-appointment'); }
...
function renderHealth() {
  ...
  return `<div class="agenda-grid">
    ...
    <div class="card"><div class="card-title">📅 Mes RDV</div>${(DB.papaAppointments || []).filter(x => daysUntil(x.date) >= 0).sort((a, b) => daysUntil(a.date) - daysUntil(b.date)).map(x => `<div class="doc-item"><div class="doc-icon">🩺</div><div class="doc-info"><div class="name">${x.type}${x.time ? ' ' + x.time : ''}</div><div class="meta">${x.doctor || ''} · ${x.notes || ''}</div></div><span class="badge badge-${daysUntil(x.date) <= 3 ? 'urgent' : 'ok'}">${fmtShort(x.date)}</span><button class="btn-del" onclick="event.stopPropagation();delFrom(DB.papaAppointments,'${x._id}')">×</button></div>`).join('') || '<div class="empty"><div class="sub">Ajoute tes RDV</div></div>'}<button class="btn btn-outline btn-full btn-sm" style="margin-top:8px" data-action="add-papa-appt">+ Mon RDV</button></div>
...
function renderAgenda() {
  ...
  return `<div class="agenda-grid">
    ...
    <div class="card"><div class="card-title">🗓 Prochains dimanches</div>${sundays.length ? sundays.map((d, i) => { const ds = dateISO(d); const ov = (DB.sundayOverrides || []).find(o => o.date === ds) || {}; const time = ov.time || s.firstSundayNote || '9h-18h'; const cancelled = ov.cancelled; return `<div class="doc-item" onclick="showSundayOverrideModal('${ds}')" style="cursor:pointer;"><div class="doc-icon">${cancelled ? '❌' : '👩‍👦'}</div><div class="doc-info"><div class="name">${fmtLong(d)}${cancelled ? ' <span style="color:var(--danger);font-size:11px;">(annulé)</span>' : ''}</div><div class="meta">${time}${ov.note ? ' · ' + ov.note : ''}</div></div><span class="badge badge-${i === 0 && !cancelled ? 'warn' : 'ok'}">${daysUntil(d)}j</span></div>`; }).join('') : '<div class="empty"><div class="sub">Configure dans ⚙️ Paramètres</div></div>'}</div>
    ...
    <div class="card"><div class="card-title">➕ Journées supplémentaires</div>
      ${(DB.extraVisits || []).length ? DB.extraVisits.sort((a, b) => new Date(a.date || 0) - new Date(b.date || 0)).map(v => `<div class="doc-item"><div class="doc-icon">👩‍👦</div><div class="doc-info"><div class="name">${fmtLong(v.date)}</div><div class="meta">${v.time ? v.time + ' · ' : ''}${v.note || ''}</div></div><button class="btn-del" onclick="event.stopPropagation();delFrom(DB.extraVisits,'${v._id}')">×</button></div>`).join('') : '<div class="empty" style="padding:8px"><div class="sub">Ajoute des jours hors dimanche (mercredi, vacances...)</div></div>'}
      <button class="btn btn-outline btn-full btn-sm" style="margin-top:8px" data-action="add-extra-visit">+ Journée</button></div>
    ${renderPastSundays()}
  </div>`;
}
...
function renderPastSundays() {
  ...
  return `<div class="card" style="grid-column:1/-1;"><div class="card-title">📊 Récap des dimanches passés</div>
    ${shown.map(p => `<div class="doc-item" onclick="showSundayOverrideModal('${p.dateStr}')" style="cursor:pointer;"><div class="doc-icon">${p.cancelled ? '❌' : '✅'}</div><div class="doc-info"><div class="name">${fmtLong(p.dateStr)}</div><div class="meta">${p.cancelled ? 'Annulé' : (p.time || '')}${p.note ? ' · ' + p.note : ''}</div></div></div>`).join('')}
    ${more ? `<details style="margin-top:8px;"><summary style="cursor:pointer;font-size:12px;font-weight:600;color:var(--primary);">Voir les ${lastAll.length - 10} dimanches précédents</summary><div style="margin-top:6px;">${lastAll.slice(10).map(p => `<div class="doc-item" onclick="showSundayOverrideModal('${p.dateStr}')" style="cursor:pointer;"><div class="doc-icon">${p.cancelled ? '❌' : '✅'}</div><div class="doc-info"><div class="name">${fmtLong(p.dateStr)}</div><div class="meta">${p.cancelled ? 'Annulé' : (p.time || '')}${p.note ? ' · ' + p.note : ''}</div></div></div>`).join('')}</div></details>` : ''}
    <div style="font-size:11px;color:var(--text-light);text-align:center;">Total: ${past.length} dimanche${past.length > 1 ? 's' : ''} · Annulés: ${past.filter(p => p.cancelled).length}</div></div>`;
}
...
function renderActivites() {
  ...
  return `<div class="agenda-grid">
    ...
    <div class="card"><div class="card-title">🎯 Ses activités</div>
      ${actAyden.length ? actAyden.map(a => `<div class="doc-item"><div class="doc-icon">${activiteIcon(a.type)}</div><div class="doc-info"><div class="name">${a.label}</div><div class="meta">${a.day || ''} ${a.time || ''} · ${a.lieu || ''}${a.coach ? ' · ' + a.coach : ''}${a.notes ? ' · ' + a.notes : ''}</div></div><button class="btn-del" onclick="event.stopPropagation();delFrom(DB.activites,'${a._id}')">×</button></div>`).join('') : '<div class="empty"><div class="sub">Foot, piscine, danse...</div></div>'}
    ...
    <div class="card"><div class="card-title">🎯 Mes activités</div>
      ${actPapa.length ? actPapa.map(a => `<div class="doc-item"><div class="doc-icon">${activiteIcon(a.type)}</div><div class="doc-info"><div class="name">${a.label}</div><div class="meta">${a.day || ''} ${a.time || ''} · ${a.lieu || ''}${a.coach ? ' · ' + a.coach : ''}${a.notes ? ' · ' + a.notes : ''}</div></div><button class="btn-del" onclick="event.stopPropagation();delFrom(DB.papaActivites,'${a._id}')">×</button></div>`).join('') : '<div class="empty"><div class="sub">Sport, musique, loisirs...</div></div>'}
...
function handleAction(a) {
  const actions = {
    ...
    'add-activite': showActiviteModal, 'add-extra-visit': showExtraVisitModal,
...
function showModal(title, fields, cb) {
  ...
  $('#mSave').onclick = () => { const d = {}; fields.forEach(f => { d[f.id] = $('#fm-' + f.id).value; }); cb(d); closeM(); };
}
...
function showSundayOverrideModal(dateStr) { const existing = (DB.sundayOverrides || []).find(o => o.date === dateStr) || { date: dateStr, time: '', note: '', cancelled: false }; showModal('Modifier le dimanche ' + fmtLong(dateStr), [{ id: 'time', l: 'Horaires', p: DB.settings.firstSundayNote || '9h-18h' }, { id: 'note', l: 'Note', p: '' }, { id: 'cancelled', l: 'Annulé ? (oui/non)', p: existing.cancelled ? 'oui' : 'non' }], d => { if (!DB.sundayOverrides) DB.sundayOverrides = []; const idx = DB.sundayOverrides.findIndex(o => o.date === dateStr); const obj = { date: dateStr, time: d.time || '', note: d.note || '', cancelled: (d.cancelled || '').toLowerCase() === 'oui' }; if (idx >= 0) DB.sundayOverrides[idx] = obj; else DB.sundayOverrides.push(obj); saveDB(); cloudPushSettings(); render(); toast('Dimanche mis à jour'); }); setTimeout(() => { $('#fm-time').value = existing.time || DB.settings.firstSundayNote || ''; $('#fm-note').value = existing.note || ''; $('#fm-cancelled').value = existing.cancelled ? 'oui' : 'non'; }, 100); }
...
function showActiviteModal() { showModal("Nouvelle activité", [{ id: "label", l: "Nom", p: "Foot, Piscine..." }, { id: "type", l: "Type", t: "sel", opts: ["Sport", "Musique", "Danse", "Piscine", "Arts", "Autre"] }, { id: "day", l: "Jour", t: "sel", opts: ["", "Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi", "Samedi", "Dimanche"] }, { id: "time", l: "Horaire", p: "14h-15h" }, { id: "lieu", l: "Lieu", p: "" }, { id: "coach", l: "Contact", p: "" }, { id: "notes", l: "Équipement / Notes", p: "" }], d => { if (!DB.activites) DB.activites = []; DB.activites.push({ _id: uid(), label: d.label, type: d.type || "Autre", day: d.day || "", time: d.time || "", lieu: d.lieu || "", coach: d.coach || "", notes: d.notes || "" }); saveDB(); cloudPushSettings(); render(); toast("Activité ajoutée"); }); }
function showExtraVisitModal() { showModal('Journée supplémentaire', [{ id: 'date', l: 'Date', t: 'date' }, { id: 'time', l: 'Horaires', p: 'ex: 10h-18h' }, { id: 'note', l: 'Note', p: 'Mercredi chez Maman, vacances...' }], d => { if (!DB.extraVisits) DB.extraVisits = []; DB.extraVisits.push({ _id: uid(), date: d.date, time: d.time || '', note: d.note || '' }); saveDB(); cloudPushSettings(); render(); toast('Journée ajoutée'); }); }
function showPapaActiviteModal() { showModal('Mon activité', [{ id: 'label', l: 'Nom', p: 'Foot, Muscu, Piscine...' }, { id: 'type', l: 'Type', t: 'sel', opts: ['Sport', 'Musique', 'Danse', 'Piscine', 'Loisir', 'Autre'] }, { id: 'day', l: 'Jour', t: 'sel', opts: ['', 'Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi', 'Dimanche'] }, { id: 'time', l: 'Horaire', p: '18h-19h' }, { id: 'lieu', l: 'Lieu', p: '' }, { id: 'notes', l: 'Notes', p: '' }], d => { if (!DB.papaActivites) DB.papaActivites = []; DB.papaActivites.push({ _id: uid(), label: d.label, type: d.type || 'Autre', day: d.day || '', time: d.time || '', lieu: d.lieu || '', notes: d.notes || '' }); saveDB(); cloudPushSettings(); render(); toast('Activité ajoutée'); }); }
function showEnsembleActiviteModal() { showModal('Activité ensemble', [{ id: 'label', l: 'Nom', p: 'Parc, Balade, Cinéma...' }, { id: 'type', l: 'Type', t: 'sel', opts: ['Parc', 'Balade', 'Cinéma', 'Sport', 'Piscine', 'Loisir', 'Autre'] }, { id: 'day', l: 'Jour', t: 'sel', opts: ['', 'Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi', 'Dimanche'] }, { id: 'time', l: 'Horaire', p: '10h-12h' }, { id: 'lieu', l: 'Lieu', p: '' }, { id: 'notes', l: 'Notes', p: '' }], d => { if (!DB.papaAydenActivites) DB.papaAydenActivites = []; DB.papaAydenActivites.push({ _id: uid(), label: d.label, type: d.type || 'Autre', day: d.day || '', time: d.time || '', lieu: d.lieu || '', notes: d.notes || '' }); saveDB(); cloudPushSettings(); render(); toast('Activité ajoutée'); }); }
