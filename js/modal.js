...
// ========== MODALS ==========
export function showModal(title, fields, cb) {
...
export function closeM() { const m = $('#modalOverlay'); if (m) m.remove(); }

export function showDocModal(scan, fileName, fileType) {
...
export function showApptModal(preDate) {
...
export function showSundayNoteModal() { const ns = getUpcomingSunday(); const defDate = ns ? dateISO(ns) : ''; showModal('Note de dimanche', [{ id: 'date', l: 'Date', t: 'date' }, { id: 'note', l: 'Note', t: 'ta', p: '' }], d => { if (!DB.sundayNotes) DB.sundayNotes = []; DB.sundayNotes.push({ date: d.date, note: d.note }); saveDB(); cloudPushSettings(); render(); toast('Note enregistrée'); }); setTimeout(() => { const el = $('#fm-date'); if (el && defDate) el.value = defDate; }, 100); }
...
export function showSchoolEditModal() {
  const sc = DB.school || {};
