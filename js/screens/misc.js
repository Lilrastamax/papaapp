...
export function renderContacts() {
  ...
  return `<div class="card"><div class="card-title">🆘 Urgences</div>${em.map(c => `<div class="doc-item"><div class="doc-icon">${c.name === 'SAMU' ? '🏥' : c.name === 'Pompiers' ? '🚒' : '👮'}</div><div class="doc-info"><div class="name">${c.name}</div><div class="meta">${c.role}</div></div><a href="tel:${c.phone}" style="font-weight:700;color:var(--danger);text-decoration:none;">${c.phone}</a></div>`).join('')}</div>
    <div class="card"><div class="card-title">👨‍⚕️ Médical</div>${med.length ? med.map(c => `<div class="doc-item" style="cursor:pointer;" onclick="editItem('contacts','${c._id}')"><div class="doc-icon">${c.specialty === 'Pédiatre' ? '🩺' : c.specialty === 'Dentiste' ? '🦷' : c.specialty === 'Ophtalmo' ? '👁️' : '💊'}</div><div class="doc-info"><div class="name">${c.name}</div><div class="meta">${c.specialty} · ${c.phone}${c.notes ? ' · ' + c.notes : ''}</div></div><a href="tel:${c.phone}" style="font-weight:600;color:var(--primary);text-decoration:none;">📞</a><button class="btn-del" onclick="event.stopPropagation();delFrom(DB.contacts,'${c._id}')">×</button></div>`).join('') : '<div class="empty"><div class="sub">Aucun contact médical</div></div>'}</div>
    <div class="card"><div class="card-title">👥 Autres</div>${other.length ? other.map(c => `<div class="doc-item" style="cursor:pointer;" onclick="editItem('contacts','${c._id}')"><div class="doc-icon">👤</div><div class="doc-info"><div class="name">${c.name}</div><div class="meta">${c.role}${c.notes ? ' · ' + c.notes : ''}</div></div><a href="tel:${c.phone}" style="font-weight:600;color:var(--primary);text-decoration:none;">📞</a><button class="btn-del" onclick="event.stopPropagation();delFrom(DB.contacts,'${c._id}')">×</button></div>`).join('') : '<div class="empty"><div class="sub">Ajoute tes contacts</div></div>'}<button class="btn btn-outline btn-full btn-sm" style="margin-top:8px" data-action="add-contact">+ Contact</button></div>`;
}

export function renderDaily() {
  const clItem = (section, prefix, i) => `<div class="cl-item"><input type="checkbox" id="${prefix}-${i.id}" ${i.checked ? 'checked' : ''} data-cl="${section}" data-id="${i.id}"><label for="${prefix}-${i.id}">${i.label}</label><button class="btn-del" onclick="renameListItem('checklists','${section}','${i.id}')">✏️</button><button class="btn-del" onclick="removeListItem('checklists','${section}','${i.id}')">×</button></div>`;
  const clAdd = (section, inputId) => `<div style="display:flex;gap:8px;margin-top:8px;"><input type="text" id="${inputId}" placeholder="Ajouter une tâche" style="flex:1;padding:8px 10px;border-radius:8px;border:1px solid var(--border);font-size:13px;"><button class="btn btn-primary btn-sm" onclick="addListItem('checklists','${section}','${inputId}')">+</button></div>`;
  return `<div class="card"><div class="card-title">🌅 Matin</div>${DB.checklists.morning.map(i => clItem('morning', 'mor', i)).join('')}${clAdd('morning', 'addMorning')}</div>
    <div class="card"><div class="card-title">🌙 Soir</div>${DB.checklists.evening.map(i => clItem('evening', 'eve', i)).join('')}${clAdd('evening', 'addEvening')}</div>
    <div class="card"><div class="card-title">🛒 Courses</div>${(DB.shoppingList || []).length ? DB.shoppingList.map((it, i) => `<div class="cl-item"><input type="checkbox" id="shop-${i}" ${it.checked ? 'checked' : ''} data-cl="shopping" data-idx="${i}"><label for="shop-${i}">${it.label}</label><button class="btn-del" onclick="renameShopItem(${i})">✏️</button><button class="btn-del" onclick="event.stopPropagation();DB.shoppingList.splice(${i},1);saveDB();cloudPushSettings();render();">×</button></div>`).join('') : '<div class="empty"><div class="sub">Ajoute des articles</div></div>'}<div style="display:flex;gap:8px;margin-top:8px;"><input type="text" id="shopInput" placeholder="Article" style="flex:1;padding:8px 10px;border-radius:8px;border:1px solid var(--border);font-size:13px;"><button class="btn btn-primary btn-sm" id="shopAdd">+</button></div></div>
    ...
    <div class="card"><div class="card-title">💎 Journal</div>${DB.memories.slice(-5).reverse().map((m, i) => `<div class="mem-card" style="cursor:pointer;" onclick="editItem('memories','${m._id}')"><div class="mem-img c${(i % 3) + 1}">${m.mood || '💎'}</div><div class="mem-body"><div class="date">${fmtLong(m.date)}</div><div class="text">${m.text}</div></div></div>`).join('') || '<div class="empty"><div class="sub">Aucun souvenir</div></div>'}<button class="btn btn-outline btn-full btn-sm" style="margin-top:4px" data-action="add-memory">+ Souvenir</button></div>`;
}
...
export function renderClothingSizes() {
  ...
  return `<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px;">${Object.entries(latest).map(([cat, c]) => `<div style="background:var(--primary-light);border-radius:12px;padding:8px 14px;text-align:center;"><div style="font-size:10px;color:var(--text-light);font-weight:600;">${cat}</div><div style="font-size:16px;font-weight:800;color:var(--primary);">${c.size || '?'}</div></div>`).join('')}</div>${ch.slice(0, 3).map(c => `<div class="doc-item" style="cursor:pointer;" onclick="editItem('clothingHistory','${c._id}')"><div style="flex:1;font-size:11px;"><b>${c.category}:</b> ${c.item || c.size}${c.outgrown ? ' <span style="color:var(--danger);">(trop petit)</span>' : ''}</div><div style="font-size:10px;color:var(--text-light);">${fmtShort(c.date)}</div><button class="btn-del" onclick="event.stopPropagation();delFrom(DB.clothingHistory,'${c._id}')">×</button></div>`).join('')}`;
}
